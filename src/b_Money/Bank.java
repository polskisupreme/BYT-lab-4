...
public class Bank {
	...
	public void openAccount(String accountid) throws AccountExistsException {
		if (accountlist.containsKey(accountid)) {
			...
			accountlist.put(accountid, new Account(accountid, currency)); //Błąd: Nie tworzyło faktycznie nowego konta, ponieważ nasze przypadki testowe zwracały obiekt null.
		}
	}
	...
	public void deposit(String accountid, Money money) throws AccountDoesNotExistException {
		if (!accountlist.containsKey(accountid)) { //Błąd: test przypadku dla metody deposit pokazał, że ten warunek if powodował błąd, jeśli istniał, a nie wtedy, gdy go nie było.
	...
	public void withdraw(String accountid, Money money) throws AccountDoesNotExistException {
		if (!accountlist.containsKey(accountid)) {
			...
			Account account = accountlist.get(accountid);
			account.withdraw(money);//Błąd: Test przypadku wypłaty pokazał, że przy wypłacie z konta o saldzie 0, zwracał wartość dodatnią zamiast ujemnej. Wiedziałem, że dodaje zamiast odejmować.
		}
	}
